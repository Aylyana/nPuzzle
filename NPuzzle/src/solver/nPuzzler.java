...
class nPuzzler
{
	...
	private static void InitMethods()
	{
		...
		lMethods[4] = new UniformCostStrategy();
	}
	...
	private static nPuzzle readProblemFile(String fileName) // this allow only one puzzle to be specified in a problem file 
	{
		...
		try
		{
			...
			int pWidth = Integer.parseInt(bothDimensions[0]);
			int pLength = Integer.parseInt(bothDimensions[1]);
			
			int[][] startPuzzleGrid = new int[pWidth][pLength];
			int[][] goalPuzzleGrid = new int[pWidth][pLength];
			
			//fill in the start state
			String startStateString = puzzle.readLine();
			startPuzzleGrid = ParseStateString(startStateString, startPuzzleGrid, pWidth, pLength);
			
			//fill in the end state
			String goalStateString = puzzle.readLine();
			goalPuzzleGrid = ParseStateString(goalStateString, goalPuzzleGrid, pWidth, pLength);
		...
		private static int[][] ParseStateString(String stateString, int[][] puzzleGrid, int pWidth, int pLength)
	{
	...
	// the top-left corner of the puzzle has a coordinate of [0,0]
		for (int y = 0; y < pLength; y++)
		{
			for (int x = 0; x < pWidth; x++)
			{
				int tileNumber = Integer.parseInt(tileLocations[(y * pWidth) + x ]);
				puzzleGrid[x][y] = tileNumber;
				System.out.println("y: " + y + " x: " + x + " value: " + tileNumber);
			}
		}
